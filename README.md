...
...
$ cd /vagrant/ansible
$ ansible-playbook -i inventories/development deploy_stack.yml -e "deploy_env=dev version=HEAD fake_ssl_cert=True"
```
...
- Update localsettings:
  ```bash
  ansible-playbook -i inventories/development deploy_stack.yml -e "deploy_env=dev version=HEAD fake_ssl_cert=True" --tags=localsettings
  ```
- Skip the common setup, including apt installs and updating the commcarehq code:
  ```bash
  ansible-playbook -i inventories/development deploy_stack.yml -e "deploy_env=dev version=HEAD fake_ssl_cert=True" --skip-tags=common
  ```
