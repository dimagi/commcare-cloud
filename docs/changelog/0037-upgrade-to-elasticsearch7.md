...
2. This is an optional step if you want to make sure that there is no lag in user models between Couch and Elasticsearch. The lag affects user based filters. 
    ...
    - If there are pending changes, you can start the relevant pillow, monitor the pending changes and stop the pillow after all the pending changes are processed.
        ...
        # to start user-pillow (replace with UserPillow if that's what you use)
          commcare-cloud <env> service pillowtop start --only user-pillow
          commcare-cloud <env> service pillowtop stop --only user-pillow
        ```
...
Forms and cases that are deleted during the reindex continue to stay in ES7.
These deletes will be reprocessed in the switchover phase.
This step requires a date after which the deletes are to be reprocessed, 
which will be the date on which you have initiated your first form/case reindex. So, make a note of that date.
