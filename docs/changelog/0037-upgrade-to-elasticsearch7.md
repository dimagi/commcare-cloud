...
1. Provision and setup a separate ES7 cluster of same size or larger than your current ES2 cluster by doing below using commcare-cloud
...
5. At this point, all of the Elasticsearch 2 data should be in Elasticsearch 7.
    - You can confirm this by running this command to display the number of documents in both ES clusters:
        ...
        ```
    - Alternatively you may use an ES monitoring tool such as https://www.elastichq.org/
...
Use below notes to reindex all of the data up to a day or two before switchover and proceed to the switchover section after that.
 ...
 2. Reindex all of form/case data up to a planned switchover date
    - Below is an example management command that reindexes forms index from Elasticsearch 2 cluster at `http://es2_cluster:9200` for a specific date range
      ...
      # an example reindex command
        ./manage.py reindex_es_from_remote forms http://es2_cluster:9200 --start_date=YYYY-MM-DD
      --end_date=YYYY-MM-DD 
      ``` 
 ...
 3. Below is a sequence of commands for an example scenario 
    ...
    ./manage.py reindex_es_from_remote forms http://es2_cluster:9200 
    --end_date=2016-01-01
...
4. Reindex offset form/case data inserted after the Reindex phase.
    ...
    ./manage.py reindex_es_from_remote cases http://es2_cluster:9200 
      --start_date=<last_reindex_date>
    ```
...
6. Reprocess form and case deletes since your first reindex from the reindex phase.
    ...
    ```
7. At this point, all of the Elasticsearch 2 data should be in Elasticsearch 7. You can confirm this by running the `reindex_es_from_remote` management command with `--print_index_size` option which displays number of documents in both the ES clusters. 
...
To get the estimate:
- execute step 5. You can see Appendix notes to reset those (not form/case) indices before actual switchover. 
