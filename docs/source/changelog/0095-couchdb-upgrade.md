...
# 95. Rolling Upgrade of CouchDB Nodes
...
2. If the node is behind a Load Balancer, deregister it from the target group now to stop incoming traffic.
3. Stop the Pillowtop services on the control machine:
   sh
...
4. *Recommended:* Take a manual snapshot of the data volume/disk before proceeding.
