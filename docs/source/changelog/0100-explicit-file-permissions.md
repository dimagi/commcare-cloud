...
- Ubuntu defaults in `/etc/login.defs` (`UMASK 022` and `USERGROUPS_ENAB yes`)
  cause the effective umask to be `002`. Therefore some permissions will
  change `0664 -> 0644` (file) and `0775 -> 0755` (directory). This only
  affects a few files because most system files have mode `0644`/`0755`.
...
provided to reduce run time while ensuring all relevant tasks are run.
