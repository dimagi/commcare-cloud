...
Enabling it in an environment requires two manual steps: set
``PGBOUNCER_AUTH_PASSWORD`` and ``PROJECTDB_USERS_PASSWORD`` in the vault.
Each domain gets its own postgres login role, provisioned from HQ using
``projectdb_provision_role`` and ``projectdb_drop_role``. All of these roles
share the single ``PROJECTDB_USERS_PASSWORD`` password; access is scoped by
which role a domain is given, not by that role having its own secret
password. RDS doesn't expose ``pg_authid``, so pgbouncer can't validate
logins against each role's real password anyway. To validate logins,
pgbouncer runs an ``auth_query`` that checks the role exists and returns
``PROJECTDB_USERS_PASSWORD``, connecting to postgres using its own
``auth_user`` and ``pgbouncer_auth`` role.
