...
@task
def ansible(deploy_env, roledefs_path):
    if env.code_branch == 'master':
    ...
    # Load roledefs dynamically from YAML file
    roledefs_steam = open(roledefs_path, 'r')
    env.roledefs = yaml.load(roledefs_steam)
...
def update_virtualenv(preindex=False):
    ...
    with cd(root_to_use):
        ...
        sudo("%s bash scripts/uninstall-requirements.sh" % cmd_prefix,
             user=env.sudo_user)
        sudo('%s pip install --allow-unverified egenix-mx-base --requirement %s --requirement %s' % (
            cmd_prefix,
