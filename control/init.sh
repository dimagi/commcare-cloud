...
if [ -z "$(which manage-commcare-cloud)" ]; then
    ...
    cd ${COMMCARE_CLOUD_REPO} && pip install pip-tools && make requirements && pip-sync  && pip install -e . && cd -
else
    { COMMCARE= && cd ${COMMCARE_CLOUD_REPO} && pip install pip-tools && make requirements && pip-sync && pip install -e . && cd - ; } &
fi
