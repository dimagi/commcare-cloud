...
{
  port=$1
  local AllPorts="SSH:22 https:443 http:80 Monolith_Commcare:9010 Formplayer:8181 Kafka:9092 Zookeeper:2181,
          Redis:6379 PostgreSQL:5432 PgBouncer:6432 RabbitMQ:5672 ElasticSearch:9200 ES_Cluster:9300 
          CouchDB:5984,4369 Celery_port:5555 Mail/SMTP:25,465,587"
  desc=${port}
  for pi in ${AllPorts} ;
  do
     if [[ "$pi" =~ .*"${port}".* ]]; then
