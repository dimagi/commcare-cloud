...
# Lambda function to add partitions for the newly created directories
resource "aws_iam_role" "waflogs_add_partitions" {
    name = "waflogs_add_partitions"
...
data "aws_iam_policy_document" "s3-access-ro-waflogs" {
    statement {
        actions = [
            "s3:GetObject",
            "s3:PutObject",
...
resource "aws_iam_policy" "s3-access-ro-waflogs" {
    name = "s3-access-ro-waflogs"
    path = "/"
    policy = data.aws_iam_policy_document.s3-access-ro-waflogs.json
}

resource "aws_iam_role_policy_attachment" "s3-access-ro-waflogs" {
    role       = aws_iam_role.waflogs_add_partitions.name
    policy_arn = aws_iam_policy.s3-access-ro-waflogs.arn
}
...
data "archive_file" "lambda_zip" {
    type          = "zip"
    source_file   = "${path.module}/waflogs_add_partitions.py"
    output_path   = "waflogs_add_partitions.zip"
}

resource "aws_lambda_function" "waflogs_add_partitions" {
    filename = "waflogs_add_partitions.zip"
    function_name = "waflogs_add_partitions"
    role = aws_iam_role.waflogs_add_partitions.arn
    handler = "waflogs_add_partitions.handler"
    runtime = "python3.8"
    timeout = 30
...
resource "aws_cloudwatch_event_rule" "check-waflogs-file-event" {
    name = "check-waflogs-file-event"
    description = "check-waflogs-file-event"
    schedule_expression = "cron(*/15 * * * ? *)"
}

resource "aws_lambda_permission" "allow_cloudwatch_to_call_waflogs" {
    statement_id = "AllowExecutionFromCloudWatch"
    action = "lambda:InvokeFunction"
    function_name = aws_lambda_function.waflogs_add_partitions.function_name
    principal = "events.amazonaws.com"
    source_arn = aws_cloudwatch_event_rule.check-waflogs-file-event.arn
}
