...
def test_convert_to_unit__time():
    ...
    assert convert_to_unit('1m', SECOND) == 60
    assert convert_to_unit('1 m', SECOND) == 60


def test_convert_to_unit__bytes():
    # 12 GB is 12 * 1024 * 1024 / 8 "8kB blocks" = 1572864 "8kB blocks"
    assert convert_to_unit('12GB', BLOCK_8KB) == 1572864
    assert convert_to_unit('12 GB', BLOCK_8KB) == 1572864
...
def test_convert_to_standard_unit__time():
    ...
    assert convert_to_standard_unit('authentication_timeout', '1m') == 60
...
def test_convert_to_standard_unit__bytes():
    ...
    assert convert_to_standard_unit('effective_cache_size', '11520MB') == 1474560
